...
function resizeCanvas() {
    let width = Math.min(window.innerWidth * 0.95, 700);
...
function drawCircle(x, y, r, color = "#00e5ff") {
    ctx.fillStyle = color;
    ctx.beginPath();
...
function draw() {
    drawRect(0, 0, WIDTH, HEIGHT, "#0d0d0d");
    drawNet();
    drawScore();
    drawRect(PLAYER_X, playerY, PADDLE_WIDTH, PADDLE_HEIGHT, "#00e5ff");
    drawRect(AI_X, aiY, PADDLE_WIDTH, PADDLE_HEIGHT, "#ff00c8");
...
function update() {
    ...
    if (ballX > WIDTH) {
        playerScore++;
        if (playerScore % 5 === 0) {
            let username = prompt("🏆 Skor tinggi! Masukkan nama kamu:");
            if (username) {
...
function submitScore(username, score) {
    fetch('https://example.com/api/score', {
        method: 'POST',
...
function loadLeaderboard() {
    fetch('https://example.com/api/leaderboard')
        ...
        .catch(() => document.getElementById('leaderboard').innerHTML = '<p>Gagal memuat leaderboard.</p>');
}
